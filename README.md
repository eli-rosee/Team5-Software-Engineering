# Team5-Software-Engineering Setup Instructions
  ...
  5. Test running the generator (can use a different terminal window and must be inside project folder. This should be ran before the game starts)
     - python3 traffic_generator.py
         - Enter all the equipment IDs for the 2 red and greens players
         - Then go back to the main.py and start the game
  ...
  6. What happens in main.py?
     ...
     - If equipment ID field is missing, the player will not be added to the game
     ...
     - After you have entered all players that you want, you can click f5 or the button labeled "f5 Start Game" to start the game
     ...
     - If it is a normal hit, the player gets 10 points for their team and individual score (-10 to the attacker if they are on the same team)
     - The middle of the screen shows all of the game actions (what player hit who)
     - The game will last 6 minutes long, and then ends the game (server broadcasts 221 to client)
     - You can return to the player entry screen as well by clicking the "Return to Player Entry Screen" button 
     - Clicking the button "Change IP" allows you to change the IP address
         - (if you do this, make sure to change these 2 lines in your traffic_generator.py get changed to the new ip you changed to in the game)
